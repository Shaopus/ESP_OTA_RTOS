...
{
    os_event_t e;
    volatile uint8 rx_stats = RX_IDLE;
...
LOCAL void
uart_config(uint8 uart_no, UART_ConfigTypeDef *uart)
{
    if (uart_no == UART1) {
        PIN_FUNC_SELECT(PERIPHS_IO_MUX_GPIO2_U, FUNC_U1TXD_BK);
    } else {
    ...
    UART_SetFlowCtrl(uart_no, uart->flow_ctrl, uart->UART_RxFlowThresh);
    UART_SetBaudrate(uart_no, uart->baud_rate);

    WRITE_PERI_REG(UART_CONF0(uart_no),
                   ((uart->parity == USART_Parity_None) ? 0x0 : (UART_PARITY_EN | uart->parity))
                   | (uart->stop_bits << UART_STOP_BIT_NUM_S)
                   | (uart->data_bits << UART_BIT_NUM_S)
                   | ((uart->flow_ctrl & USART_HardwareFlowControl_CTS) ? UART_TX_FLOW_EN : 0x0)
                   | uart->UART_InverseMask);
...
void
uart_init(UART_BautRate  uart0_br, UART_BautRate  uart1_br)
{
    ...
    xTaskCreate(uart_task, (uint8 const *)"uTask", 512, NULL, 3, &xUartTaskHandle);
}

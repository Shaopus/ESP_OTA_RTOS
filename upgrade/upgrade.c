...
const char* demo_ip = "192.168.0.106";
...
void upgrade_task(void *pvParameters)
{
    ...
    while (retry_count++ < UPGRADE_RETRY_TIMES) {
        ...
        /*for upgrade connection debug*/
//        server->sockaddrin.sin_addr.s_addr= inet_addr("192.168.0.106");

        bzero(&remote_ip,sizeof(struct sockaddr_in));
        remote_ip.sin_family=AF_INET;
        remote_ip.sin_addr.s_addr= inet_addr(demo_ip);
